...
app.post('/comprar', async(req,res)=>{
    ...
    for (const item of dados_carrinho){
        ...
        console.log(produto)
        if(!produto || produto.quantidade_estoque < item.qtde){
            return res.status(400).json({message:"Produto não Disponível" + produto.quantidade_estoque})
        }
        const atualiza_promessas = await Produto.update(
            {quantidade_estoque: produto.quantidade_estoque - item.qtde},
